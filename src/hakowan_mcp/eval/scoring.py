...
def _tokens(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for value in _walk(payload):
        ...
        if isinstance(value.get("name"), str) and "scales" in value and "unit" in value:
...
def _fraction(required, actual) -> float:
    ...
    return len(required_set & set(actual)) / len(required_set) if required_set else 1.0
...
def patch_minimality(
    ...
    unrelated = (
        sum(
            not any(
                str(operation.get("path", "")).startswith(prefix)
