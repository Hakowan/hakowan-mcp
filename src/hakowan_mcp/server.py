...
explicitly requests an interactive viewer. For wireframes, overlay a sized curve
mark on the original mesh; do not use the `boundary` transform, which extracts
only topological boundaries. Add analytical observation passes only when visual
evidence is needed. Keep WebGL `offline=false` unless the user
