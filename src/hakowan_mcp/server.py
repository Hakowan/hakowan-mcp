...
explicitly requests an interactive viewer. For wireframes, overlay a curve mark
on the original mesh with size `0.01` unless the user requests another visible
width; do not use the `boundary` transform, which extracts only topological
boundaries. Add analytical observation passes only when visual evidence is
needed. Keep WebGL `offline=false` unless the user
